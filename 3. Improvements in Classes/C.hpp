...
//CONSTRUCTORS AND DESTRUCTOR
    ...
    //Move constructor
    //Move constructor will only be called when an rvalue expression is passed to it, and there will be no implicit conversion from an lvalue to an rvalue
    //Therefore, no need for 'explicit'
    constexpr C(C&& source) noexcept: m_vector(std::move(source.m_vector))
    {
        std::cout << "Move constructor in C class has been used." << std::endl;
    };
