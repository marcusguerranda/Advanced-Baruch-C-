...
void SalonManager::Run(std::shared_ptr<SalonManager>& shared_manager)
{
    ...
    while(BarberCondition())
    {
        ...
        { 
            ...
            barberThreads.emplace_back([this, &shared_manager, &id]() 
            {

                Barber b(id, std::ref(m_queue), shared_manager);
                b(); // Call operator()
            });
        ...
        while (m_SalonOpen.test())
        {
            ...
            customerThreads.emplace_back([this, &shared_manager, &customerId]() 
            {
                std::cout << std::boolalpha;
                std::cout << "MY GODDAMN SALON IS OPEN: " << this->IsOpen()<< std::endl;
                Customer c(customerId, std::ref(m_queue), std::ref(shared_manager));
                c(); // Call operator()
            });
            customerId++;
